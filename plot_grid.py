...
def main_sweep2():
    ...
    for f in tqdm(glob("/home/homedir/w_disc_again_easy_stuff/*/progress.csv")):
        ...
        try:
            ...
            if len(data["eprewmean"]) > 2000:
                data["eprewmean"] = (
                    data["eprewmean"]
                    .groupby(np.arange(len(data["eprewmean"])) // 2)
                    .mean()
                )
            if len(data["eval_eprewmean"]) > 2000:
                data["eval_eprewmean"] = (
                    data["eval_eprewmean"]
                    .groupby(np.arange(len(data["eval_eprewmean"])) // 2)
                    .mean()
                )
            sns.lineplot(
                data=data[["eprewmean", "eval_eprewmean"]], palette=["blue", "orange"]
            )

            data["eprewmean"] = (data["eprewmean"] - mins[env_name]) / (
                maxes[env_name] - mins[env_name]
            )
            my_normalized_returns_train.append(data["eprewmean"])
            data["eval_eprewmean"] = (data["eval_eprewmean"] - mins[env_name]) / (
                maxes[env_name] - mins[env_name]
            )
            my_normalized_returns_test.append(data["eval_eprewmean"])

            sns.lineplot(
                data=reported_data[["train_mean0", "test_mean2"]],
                palette=["green", "red"],
            )

            data = data
            reported_data = reported_data
            reported_data["train_mean0"] = (
                reported_data["train_mean0"] - mins[env_name]
            ) / (maxes[env_name] - mins[env_name])
            reported_data["test_mean2"] = (
                reported_data["test_mean2"] - mins[env_name]
            ) / (maxes[env_name] - mins[env_name])
    ...
    my_normalized_returns_train = pd.DataFrame.from_dict(
        map(dict, my_normalized_returns_train)
    ).mean(axis=0)
    my_normalized_returns_test = pd.DataFrame.from_dict(
        map(dict, my_normalized_returns_test)
    ).mean(axis=0)
    reported_normalized_returns_train = pd.DataFrame.from_dict(
        map(dict, reported_normalized_returns_train)
    ).mean(axis=0)
    reported_normalized_returns_test = pd.DataFrame.from_dict(
        map(dict, reported_normalized_returns_test)
    ).mean(axis=0)
    final_mean = pd.DataFrame.from_dict(
        (
            [
                my_normalized_returns_train,
                my_normalized_returns_test,
...
def main_sweep():
    AVG_LEN = 100
    for f in tqdm(glob("/home/homedir/procgen_wdisc_easy/*/progress.csv")):
        if os.stat(f).st_size == 0:
            continue
        try:
            data = pd.read_csv(f)
...
def main_trials():
    ...
    for thing in things:
        ...
        for f in tqdm(
            glob(
                "/home/homedir/jumper_easy_gan_manytrials/*" + thing + "*/progress.csv"
            )
        ):
            if os.stat(f).st_size == 0:
                continue
